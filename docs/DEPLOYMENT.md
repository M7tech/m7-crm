...
The production asset build runs `scripts/prepare-browser-ocr.mjs` and copies the worker, WebAssembly core, and language files to `/public/ocr/v7-2`. After deployment, confirm `/ocr/v7-2/worker.min.js` is reachable and test a card from a mobile browser. Review the extracted fields and either select an existing CRM client company or create a new client company with this first contact. A client company created here is a customer record only and receives no login or workspace. Cancelling, navigating away, or saving releases the browser's temporary image; the user's original gallery photo remains unchanged.
